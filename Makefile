# DWELL.fyi — convenience commands for local dev.
#
...
DATABASE_URL ?= postgresql://postgres:postgres@localhost:5432/dwell
export DATABASE_URL
...
mac-demo:
	cd $(MAC_DIR) && DWELL_DEMO=1 swift run SponsorOverlay

## mac-probe: Run the macOS app in probe mode to verify generation detection.
mac-probe:
	cd $(MAC_DIR) && DWELL_PROBE=1 swift run SponsorOverlay
