...
/// Deploys the full AIAD stack in dependency order and wires it together.
///
/// Required env:
...
contract Deploy is Script {
    function run() external {
        ...
        AIAD aiad = new AIAD(treasury);
        MerkleRewardsDistributor distributor =
            new MerkleRewardsDistributor(IERC20(address(aiad)), treasury, rootSetter);
        CampaignFunder funder = new CampaignFunder(
            IERC20(usdc), IAIAD(address(aiad)), address(distributor), treasury, treasury
        );
