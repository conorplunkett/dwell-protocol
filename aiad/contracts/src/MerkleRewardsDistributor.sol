...
/// @title MerkleRewardsDistributor — cumulative Merkle claims for AIAD rewards
/// @notice The backend accrues each user's AIAD entitlement offchain (at the
...
contract MerkleRewardsDistributor is Ownable2Step, Pausable, ReentrancyGuard {
    ...
    constructor(IERC20 aiad_, address owner_, address rootSetter_) Ownable(owner_) {
        require(address(aiad_) != address(0), "distributor: aiad is zero");
        ...
        whenNotPaused
    {
        ...
        aiad.safeTransfer(account, owed);
