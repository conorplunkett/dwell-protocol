...
**Get paid for your attention.** AIAD is the token-payout successor to
[freeai.fyi](https://freeai.fyi): the same product — one sponsored line shown
while an AI assistant is thinking — but the viewer's revenue share is paid in
**AIAD points** today and the **AIAD token** after launch, instead of gift-card
credits.
...
This folder is fully self-contained (no code dependencies on the parent repo)
and is structured to be lifted into its own repository unchanged. It currently
lives alongside freeai.fyi, whose backend it adapts — see
[docs/04-backend-adaptation.md](docs/04-backend-adaptation.md) for the exact
mapping.
...
| Path | What it is |
|---|---|
| [`docs/`](docs/) | The design: tokenomics, architecture, providers, backend adaptation, legal, launch checklist |
| [`contracts/`](contracts/) | Solidity (Foundry + pinned OpenZeppelin v5.6.1): the AIAD token, CampaignFunder, MerkleRewardsDistributor |
| [`web/`](web/) | The site: landing page + user portal. Static HTML/CSS/vanilla JS, no build step — freeai.fyi's design language on the AIAD black/green theme |
...
4. [Backend adaptation](docs/04-backend-adaptation.md) — precise mapping onto
   the freeai.fyi server + edge function
...
while this folder lives in the freeai.fyi repo).
...
that this sandbox had no Solidity toolchain, so a green `forge test` is a
merge gate, not an already-banked result.
