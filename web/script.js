// DWELL landing script. Self-contained: the page renders fully standalone, and
...
// The escrowed pool then splits 60% viewer / 10% referrer / 30% protocol
// treasury (held, never sold); when a viewer has no referrer, the 10%
// referrer leg joins the treasury.
...
const ECONOMICS = {
  ...
  split: { viewer: 0.60, referrer: 0.10, treasury: 0.30 },
  pointsPerDollar: 1000, // 1,000 points = $1.00 of earned ad value
};

// Derived per-$100 dollars (Viewer $54.00 / Referrer $9.00 / Treasury $27.00).
const POOL_USD = {
...
// views, one view is worth (DEMO_CPM/1000) × 90% × 60% to the viewer, ×1000
// for points — $15 CPM → 8.10 points per view.
const DEMO_CPM = 15;
const PTS_PER_VIEW =
...
// outstanding derive from the escrow: points are minted to viewers (60%) and
// referrers (10%) — 70¢ of every escrowed pool dollar — so each escrowed
...
(function reserveTicker() {
  ...
  if (DEV_MODE) {
    ...
    if (!prefersReducedMotion) {
      const rnd = mulberry32(0x41494144); // "DWELL" — same sequence every load
      setInterval(() => {
...
// the ECONOMICS split live: views, the escrowed 90%, and the viewers' 60% of
