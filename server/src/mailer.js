...
function createMailer(config) {
  ...
  async function send(to, subject, htmlBody, opts = {}) {
    ...
    console.log(`[dwell][mail] to=${to} subject="${subject}" from=${from}${link ? ` link=${link}` : ""}`);
  }
  ...
  async function sendWebLoginEmail(to, link) {
    await send(to, "Your DWELL sign-in link", shell({
  ...
  async function sendAdvertiserReceiptEmail(to, { campaignId, brand, adLine, pricePerBlockCents, blocks }) {
    await sendAds(to, "Your DWELL campaign receipt", shell({
  ...
  async function sendCampaignRejectedEmail(to, { campaignId, brand, adLine, pricePerBlockCents, blocks, note }) {
    await sendAds(to, "Your DWELL campaign was refunded", shell({
  ...
  // Invite a friend to DWELL. Sent to the invitee from the dashboard's
  ...
  async function sendReferralInviteEmail(to, { inviterEmail, link, rewardUsd }) {
    await send(to, `${inviterEmail} invited you to DWELL — free Claude credits`, shell({
      preheader: `${inviterEmail} invited you to DWELL — earn free Claude credits.`,
      hero: "🎁", heading: "You're invited to DWELL",
      body: `<p style="margin:0 0 14px;"><strong style="color:#1f1e1d;">${inviterEmail}</strong> is earning free Claude credits with DWELL and wants you in.</p>`
  ...
  async function sendCrewInviteEmail(to, { inviterEmail, link, rewardPct }) {
    await send(to, `${inviterEmail} added you to their DWELL crew`, shell({
  ...
  async function sendReferralRewardEmail(to, { rewardUsd, link }) {
    await send(to, `You earned $${Math.round(rewardUsd)} in Claude credits 🎉`, shell({
      ...
      body: `<p style="margin:0 0 14px;">A friend you referred just redeemed their first Claude gift card on DWELL — so we've added a one-time <strong style="color:#1f1e1d;">$${Math.round(rewardUsd)}</strong> bonus to your balance. 🙌</p>`
  ...
  async function sendWaitlistConfirmationEmail(to) {
    await send(to, "You're on the DWELL waitlist 🎉", shell({
