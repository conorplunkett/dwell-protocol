#!/usr/bin/env python3
"""Generate the DWELL app/extension icons — the "D$" coral wordmark — for every
...
The mark is the monospace "D$" (JetBrains Mono, the same face as the site .logo
...
# Layout — the mark is the monospace "D$" used by the site logo (.logo, which is
...
def main():
    chrome = find_chrome()
    grad = read_gradient()
    print(f"D$ coral mark  gradient {grad[0]}→{grad[1]}  via {chrome}")
