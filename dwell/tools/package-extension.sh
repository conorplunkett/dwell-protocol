#!/usr/bin/env bash
# Package the DWELL Chrome extension into a clean, Web Store-ready .zip.
#
# Produces chrome-extension/dist/dwell-chrome-v<version>.zip containing ONLY
...
mkdir -p "$DIST"
OUT="$DIST/dwell-chrome-v$VERSION.zip"
rm -f "$OUT"
# -X drops extra file attributes for a reproducible, minimal archive.
zip -q -X "$OUT" "${FILES[@]}"

echo "Packaged DWELL Chrome extension v$VERSION (${#FILES[@]} files, $(du -h "$OUT" | cut -f1))"
echo "  -> $OUT"
echo ""
printf '  %s\n' "${FILES[@]}"
echo ""
