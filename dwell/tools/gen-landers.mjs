...
const ADV_MAIN = `<main id="top">
    ...
    <section class="faq" id="faq">
      ...
      <details class="faq-item">
        <summary class="faq-q">How do you know if views are legitimate?</summary>
        <div class="faq-a">
          <p class="faq-lead">Every impression is <strong>server-issued and single-use</strong>. When your ad is about to show, our server picks the auction winner and mints a one-time token tied to that specific ad, device and moment — so a client can never invent or inflate a count, it can only redeem a token we handed out.</p>
          <p class="faq-lead">A token bills only after the ad has been <strong>on the user's screen for at least two continuous seconds</strong> — the AI assistant focused and generating, the line in view, not covered and not a background tab — and each token redeems exactly once. Replays, forged counts and redemptions that come in too fast are rejected on our server, and per-person and per-network daily caps stop anyone padding their own views. You pay for real attention, not bot traffic.</p>
        </div>
      </details>
