...
/// preconfigured amount of DWELL. `payout`/`pull` are set per test to model
...
contract MockSwapTarget {
    ...
    constructor(IERC20 usdc_, IERC20 dwell_) {
        usdc = usdc_;
        dwell = dwell_;
    }
    ...
    function swap() external {
        ...
        dwell.transfer(msg.sender, payout);
    }
}

contract CampaignFunderTest is Test {
    MockUSDC internal usdc;
    DWELL internal dwell;
    ...
    function setUp() public {
        usdc = new MockUSDC();
        dwell = new DWELL(treasury);
        funder = new CampaignFunder(usdc, IDWELL(address(dwell)), distributor, treasury, owner);
        swapTarget = new MockSwapTarget(usdc, dwell);
        ...
        vm.prank(treasury);
        dwell.transfer(address(swapTarget), 10_000_000e18); // router liquidity
    ...
    function test_splitsSeventyThirty() public {
        _fund(TRANCHE, 45_000e18, 45_000e18); // $90 at $0.002 → 45,000 DWELL

        assertEq(dwell.balanceOf(distributor), 31_500e18); // 70%
        assertEq(dwell.balanceOf(treasury) - (1_000_000_000e18 - 10_000_000e18), 13_500e18); // 30%
        assertEq(funder.fundedCampaigns(CAMPAIGN), 45_000e18);
        assertEq(funder.totalUsdcSpent(), TRANCHE);
        assertEq(funder.totalDwellBurned(), 0);
        // approvals reset
    ...
    function test_burnBpsBurnsTreasurySlice() public {
        ...
        uint256 supplyBefore = dwell.totalSupply();

        _fund(TRANCHE, 45_000e18, 45_000e18);

        uint256 expectedBurn = (13_500e18 * 2_000) / 10_000;
        assertEq(dwell.totalSupply(), supplyBefore - expectedBurn);
        assertEq(funder.totalDwellBurned(), expectedBurn);
        assertEq(dwell.balanceOf(distributor), 31_500e18); // distributor leg untouched
    }
    ...
    function test_rescueBlocklistsFlowTokens() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(CampaignFunder.RescueBlocked.selector, address(dwell))
        );
        funder.rescue(address(dwell), owner, 1);
        vm.stopPrank();
