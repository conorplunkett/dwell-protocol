...
How the parent-repo backend becomes the DWELL backend. **Status: §A–§D are
implemented** (2026-07-06) in the shared backend code — `server/db/schema.sql`
+ `server/db/20260706_dwell_token_mode.sql`, `server/src/`, and the edge
...
DWELL deployment is behavior-identical to before (two-way split, no token
machinery, token routes 404); the DWELL deployment enables points mode via
...
the production Supabase Edge Function (`supabase/functions/dwell-api/index.ts`),
...
(impression tokens, dwell backstop, daily caps). DWELL adds entry types, a few
...
DWELL replaces it with a three-way BPS split **of the 90% tranche**:
...
**Retired in DWELL**: the `creditAffiliate()` platform-funded 10% bonus
...
|---|---|---|
| `TOKEN_MODE` | *(unset)* | The deployment switch: unset = legacy DWELL behavior (token routes 404); `points` = accrual phase; `live` = post-TGE. The DWELL deployment sets `points` at launch. |
...
| `BRAND_NAME` / `STRIPE_PRODUCT_NAME` / `STRIPE_PRODUCT_IMAGE` | DWELL values | Brand knobs: the DWELL deployment bills its Stripe checkouts under its own product name/image (its own Stripe *account* is the Separation-list plan; these work either way) |
