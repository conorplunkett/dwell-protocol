...
```
 advertiser ──card──▶ Stripe ──webhook──▶ campaign paid
                              ...
                              CampaignFunder.swapAndFund(campaignId, usdcIn, minOut, 0x calldata)
                                    ...
                                    ├──▶ 70% of DWELL → MerkleRewardsDistributor (user-claimable)
                                    ├──▶ 30% of DWELL → protocol treasury Safe (held, never sold;
                                    │       unreferred 10% legs settle to it via a Merkle shortfall leaf)
                                    └──▶ emits CampaignFunded(campaignId, usdcIn, dwellOut)
                                              │
                                              ▼
                              token_campaign_pools row → locked rate = dwellOut ÷ impressions

 user views ad ──same impression pipeline──▶ ledger accrual in DWELL units at the locked rate
 root publisher (cron): builds cumulative (address, cumulativeAmount) tree over accrued DWELL
     → MerkleRewardsDistributor.setRoot(root, epoch+1)
 user claims: portal fetches proof (GET /v1/web/token/claim-proof) → claim() → wallet
 cash-out: user swaps DWELL→USDC on the DEX and offramps via licensed partner
...
|---|---|---|---|
| Issuer treasury Safe (multisig) | DWELL allocations, protocol treasury | 2-of-3: founder + 2 trusted signers (hardware) | Whole treasury — highest protection |
