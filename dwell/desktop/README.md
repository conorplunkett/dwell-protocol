...
**dwells** for qualified impressions (see
[`../docs/01-tokenomics.md`](../docs/01-tokenomics.md) for how dwells are
valued and redeemed). It never injects code into the app, never modifies its
...
The app speaks the **existing DWELL API** (`server/`), the same protocol
as the other DWELL clients: `POST /v1/devices/register` (anonymous device auth),
...
Enabled card. The sign-in step opens DWELL's web sign-in in the browser; the rail
steps are clickable to revisit any completed step (their ✓ persists); and the final
"Open DWELL" button closes the window and pops open the menu-bar item's menu.
...
> `DWELL_API_URL=http://localhost:8787` and follow [`../DEVNET.md`](../DEVNET.md)
...
```sh
DWELL_PROBE=1 swift run SponsorOverlay
```
...
Set `DWELL_PROBE_VERBOSE=1` to dump *every* labeled element/button instead
...
```sh
DWELL_DEMO=1 swift run SponsorOverlay    # or plain `swift run` with Claude open
```
...
```sh
swift run SponsorOverlay                  # uses DWELL_API_URL or the default API
```
...
2. Stash notary credentials in the Keychain so you never paste a password again —
   ...
   ```sh
   xcrun notarytool store-credentials dwell \
...
5. In-app redemption UI. Redemption today happens in the web portal
   (`dwellprotocol.com/portal`): USDC to a linked wallet, or Claude credits
   (see [`../docs/01-tokenomics.md`](../docs/01-tokenomics.md)).
