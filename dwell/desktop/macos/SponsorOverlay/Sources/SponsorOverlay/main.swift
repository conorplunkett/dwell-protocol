// DWELL Sponsor Overlay — menu bar shell.
//
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let demoMode = ProcessInfo.processInfo.environment["DWELL_DEMO"] == "1"
    // DWELL_PROBE=1: every 2s, dump the labeled elements of the focused
    ...
    private let probeMode = ProcessInfo.processInfo.environment["DWELL_PROBE"] == "1"
    ...
    private func installEditMenu() {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(withTitle: "Quit DWELL", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
    ...
    /// the user toggles DWELL on in System Settings.
    private var permissionPollTimer: Timer?
    private let onboardingMessage = "dwell"   // JS message-handler name
    ...
    private func pushPermissionState() {
        ...
        evalOnboardingJS("window.dwellBridge && window.dwellBridge.setPermission('\(granted ? "ok" : "off")')")
    }

    private func pushLaunchState() {
        evalOnboardingJS("window.dwellBridge && window.dwellBridge.setLaunchState(\(launchAtLoginEnabled() ? "true" : "false"))")
    }
    ...
    private func pushLinkedToOnboarding() {
        ...
        evalOnboardingJS("window.dwellBridge && window.dwellBridge.setLinked && window.dwellBridge.setLinked('\(state)', '\(email)')")
    }
    ...
    private func setLaunchAtLogin(_ enabled: Bool) {
        ...
        do {
            ...
            NSLog("[dwell] launch-at-login \(enabled ? "register" : "unregister") failed: \(error.localizedDescription)")
        }
    }
...
extension AppDelegate: WKScriptMessageHandler {
    func userContentController(_ userContentController: WKUserContentController,
        ...
        case "finish":
            ...
            // item so they see where DWELL now lives.
