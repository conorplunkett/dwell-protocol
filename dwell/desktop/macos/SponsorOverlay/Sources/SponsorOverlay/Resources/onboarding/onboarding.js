 ...
 *   app → JS: window.dwellBridge.setPermission(state) / setLaunchState(on)
...
(function () {
  ...
  // ── Native bridge ──
  var hasNative = !!(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.dwell);
  function native(action, payload) {
    if (!hasNative) return false;
    try {
      var msg = { action: action };
      if (payload) for (var k in payload) msg[k] = payload[k];
      window.webkit.messageHandlers.dwell.postMessage(msg);
      return true;
