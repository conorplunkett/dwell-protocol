// HTTP client for the existing DWELL API (server/src/app.js). The
// desktop app speaks the same protocol as the other DWELL clients:
...
final class BackendClient {
    ...
    static let defaultBaseURL = "https://wpjfhezklpczxzocgxsb.supabase.co/functions/v1/dwell-api"
    static var configuredBaseURL: URL {
        let override = ProcessInfo.processInfo.environment["DWELL_API_URL"]
    ...
    private func post(_ path: String, body: [String: Any],
        ...
        session.dataTask(with: req) { data, resp, err in
            ...
            guard let http = resp as? HTTPURLResponse, (200..<300).contains(http.statusCode),
                ...
                return completion(.failure(NSError(domain: "dwell", code: code)))
            }
    ...
    private func get(_ path: String, query: [String: String] = [:],
        ...
        session.dataTask(with: comps.url!) { data, resp, err in
            ...
            guard let http = resp as? HTTPURLResponse, (200..<300).contains(http.statusCode), let data else {
                return completion(.failure(NSError(domain: "dwell", code: (resp as? HTTPURLResponse)?.statusCode ?? -1)))
            }
