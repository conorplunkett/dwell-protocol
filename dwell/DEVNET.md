...
A turnkey way to run the **whole DWELL stack locally** and watch a real account
...
> (`[dwell][mail] … link=…`), so you can complete login flows locally with no
> mail provider.
...
| **Claude Code terminal** | `DWELL_BASE=http://localhost:8787` | `dwell claude` then connect email in-app |
| **macOS app** | `DWELL_API_URL=http://localhost:8787 make mac-run` | connect email in the menu-bar app |
...
| **Site / portal** | set `<meta name="dwell-api">` (or serve with the dev base) | sign in at `/redeem.html` |
