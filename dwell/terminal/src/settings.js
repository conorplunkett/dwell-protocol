...
// The only settings file whose statusLine DWELL is allowed to CHAIN (re-execute).
...
// execution behind its folder-trust prompt, but DWELL runs the chained command
...
export function isForeignStatusLine(value) {
  return isPlainObject(value)
    ...
    && !value.command.includes("dwell-statusline")
    && !value.command.includes("dwell claude statusline");
}
...
export function buildDwellStatusLine(params) {
  const statusLine = {
    type: "command",
