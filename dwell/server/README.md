# DWELL API — the credit loop

> **Production now runs as a Supabase Edge Function** (`supabase/functions/dwell-api/`),
...
```bash
DATABASE_URL=postgresql://postgres:postgres@localhost:5432/dwell npm test
```
...
```json
{ "dwell.serverUrl": "http://localhost:8787" }
```
...
> Edge Function in `supabase/functions/dwell-api/` (a verbatim port); the site is on
