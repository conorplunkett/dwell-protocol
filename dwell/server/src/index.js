...
boot()
  .then(({ deps, pool }) => {
    ...
    server.listen(deps.config.port, () => {
      console.log(`[dwell] api on :${deps.config.port} — developers keep ${deps.config.revenueShare * 100}%`);
    });
    // periodic rate-limiter sweep
    const sweep = setInterval(() => deps.rateLimiter.sweep(), 300000).unref();

    async function shutdown(sig) {
      console.log(`[dwell] ${sig} — draining…`);
      clearInterval(sweep);
      server.close(async () => {
  ...
  .catch((err) => {
    console.error("[dwell] failed to boot:", err.message);
    process.exit(1);
  });
